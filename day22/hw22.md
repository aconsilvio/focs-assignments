...
[x] I completed this assignment with assistance from https://en.wikipedia.org/wiki/Line_wrap_and_word_wrap#Algorithm
```
...
I believe this is a greedy algorithm because it optimizes each line first before moving on to the next line.  It doesn't consider its function or goal as a whole, just one line at a time.

b. If the text contains a word that is longer than `max_width`, what does this algorithm do?

The algorithm just prints that word on a new line and breaks the max_width length.

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?)

If the optimal result is the shortest number of lines, yes.  If the optimal result is minimum raggedness, then no.
...
This algorithm is an example of bottom up dynamic programming.
